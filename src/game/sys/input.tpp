...
template <typename GameCTX_t>
void
InputSystem_t<GameCTX_t>::onkeyrelease(KeySym k) {
...
InputSystem_t<GameCTX_t>::update(GameCTX_t& g) const {
  ...
  for (auto& inp :
       g.template getComponents<InputComponent_t>()) {
    auto* e = g.getEntityByID(inp.getEntityID());
    if (e) {
      auto* phy =
          e->template getComponent<PhysicsComponent_t>();

      if (phy) {
        phy->vx = phy->vy = 0;
        if (ms_Keyboard.isKeyPressed(inp.key_LEFT))
          phy->vx = -1;
        if (ms_Keyboard.isKeyPressed(inp.key_RIGHT))
          phy->vx = 1;
