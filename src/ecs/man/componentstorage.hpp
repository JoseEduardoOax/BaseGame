...
struct ComponentStorage_t {
  ...
  ComponentStorage_t&
  operator=(const ComponentStorage_t&) = delete;
  ComponentStorage_t&
  operator=(ComponentStorage_t&&) = delete;
  ...
  template <typename CMP_t> Vec_t<CMP_t>& getComponents() {
    ...
    if (it != m_componentVectors.end()) {
      auto* v = dynamic_cast<ComponentVector_t<CMP_t>*>(
          it->second.get());
      comvec = &(v->components);
    } else {
...
private:
  Hash_t<ComponentTypeID_t, Uptr<ComponentVectorBase_t>>
      m_componentVectors;
